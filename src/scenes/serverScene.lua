...
function ServerScene:sendPlayerData(peer, player_data)
    self.udp_server:send(peer, { event = "player_data", player_data = player_data })
end

function ServerScene:sendChunks(peer, chunks)
    self.udp_server:send(peer, { event = "load_chunks", chunks = chunks })
end

-- sends the chunk data that this tile is in to players within 1 chunk away
function ServerScene:sendTileChanged(world, tile_x, tile_y)
    ...
    for _, player in pairs(world.players) do
        ...
        if player_chunk_x - cx <= chunk_distance and player_chunk_y - cy <= chunk_distance then
            self:sendChunks(player.peer, updated_chunks)
        end
    end
end

function ServerScene:getPlayerFromPeer(peer)
...
function ServerScene:load()
    ...
    local ServerEvents = {
        ...
        player_auth = function(event, data)
            ...
            self:sendPlayerData(event.peer, player:getData()) -- send their player data back to owner
            ...
            self:sendChunks(event.peer, chunks_to_send)
                ...
                if hit then
                    ...
                    if tile_instance then
                        ...
                        if tile_damage >= 1 then
                            ...
                            self:sendTileChanged(self.world, tx, ty)
                        else
        ...
        player_inventory_move = function(event, data)
            ...
            if player then
                ...
                self:sendPlayerData(player.peer, player:getData())
            end
        end,
    ...
    self.udp_server = UdpServer:new(SERVER_CONFIG.hosting_ip, ServerEvents)
end

function ServerScene:update(dt)
    ...
    if self.time > self.update_rate then
        for token, player in pairs(self.world.players) do
            ...
            if next(chunks_to_send) then self:sendChunks(player.peer, chunks_to_send) end
            ...
            if next(nearby_players) then
                self.udp_server:send(player.peer, {
                    event = "update_players",
        ...
        for tile_coord, tile_instance in pairs(chunk_data) do
            if tile_instance.damage and tile_instance.last_damaged then
                if self.world.properties.ticks - tile_instance.last_damaged >= 3 then
                    ...
                    self:sendTileChanged(self.world, tx + cx * CHUNK_WIDTH, ty + cy * CHUNK_HEIGHT)
                end
            end
        end
    end
...
function ServerScene:draw()
    ...
    local connectedClients = self.udp_server:getClients()
